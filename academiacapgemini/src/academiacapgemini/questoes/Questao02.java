...
public class Questao02 {
	...
	public static void validaSenha(String senha) {
		...
		if (senha != null) {
			...
			if (senha.length() < 6) {
				System.out.println("A SENHE DEVE CONTER NO MINIMO 6 CARACTERS.");

			} else if (!Questao02.validaCaractersSenha(senha, 1)) {
				System.out.println("A SENHE DEVE CONTER NO MINIMO 1 CARACTER MAIUSCULO.");

			} else if (!Questao02.validaCaractersSenha(senha, 2)) {
				System.out.println("A SENHE DEVE CONTER NO MINIMO 1 CARACTER MINUSCULO.");

			} else if (!Questao02.validaCaractersSenha(senha, 3)) {
				System.out.println("A SENHE DEVE CONTER NO MINIMO 1 DIGITO.");

			} else if (!Questao02.validaCaractersSenha(senha, 4)) {
				System.out.println("A SENHE DEVE CONTER NO MINIMO 1 CARACTER ESPECIAL EX. '!@#$%^&*()-+'.");
